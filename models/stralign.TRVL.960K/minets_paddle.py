...
def set_global_initializer(weight_fn, bias=0.0):
    """ Done in MyEmbeddingLayer, not sure whether it is the best/correct place for it """
...
def get_norm_layer(**kwargs):
    ...
    if norm_layer.is_None:
        ...
        # MyNormLayer accepts seqs_len, but paddle Norms don't...
        return norm_layer
...
def stack_linear_block(channel_sizes, data_fmt=None, pre_act_norm=False, act_after_norm=False,
    ...
    for i in range(1, num_layers + 1):
        ...
        for house_type in house_order:
            ...
            if house_type == 'act_norm':
                room_order = ['act', 'norm']
                if act_after_norm: room_order = room_order[::-1]

                for room_type in room_order:
                    if room_type == 'act':
                        block_layers.append(get_act_layer(act_fn))
                    if room_type == 'norm':
                        block_layers.append(get_norm_layer(**norm_args, in_channels=in_channels,
                            data_fmt=data_fmt))

            if house_type == 'dropout':
...
def stack_conv1d_block(channel_sizes, data_fmt=None, kernel_size=None, stride=None, dilation=None,
    ...
    for i in range(1, num_layers + 1):
        ...
        house_order = ['conv1d_maxpool', 'act_norm']
        if pre_act_norm: house_order = house_order[::-1]
        house_order += ['dropout']
        for house_type in house_order:
...
def stack_conv2d_block(channel_sizes, data_fmt=None, kernel_size=None, stride=None, dilation=None,
    ...
    in_channels = channel_sizes[0]
    for i in range(1, num_layers + 1):
        house_order = ['conv2d_maxpool', 'act_norm']
        if pre_act_norm: house_order = house_order[::-1]
        house_order += ['dropout']
        for house_type in house_order:
...
class MyLSTM1DTower(nn.Layer):
    def __init__(self, args, in_channels=None, data_fmt='NLC', lstm_num=None, lstm_dim=None,
                is_return=False, initial_states=None, return_states=False):
    ...
    def forward(self, x, seqs_len):
        ...
        # seqs_len = mi.full((batch_size,1), input_len, dtype='int32')
...
class MyAttnxDTower(nn.Layer):
    def __init__(self, args, in_channels=None, data_fmt=None, attn_dual=False,  attn_trio=False,
        ...
        for i, AttnLayer in enumerate(self.AttnLayers):
            self.add_sublayer(f'AttnLayer{i}', AttnLayer)

        if self.norm_out:
            self.NormOut = get_norm_layer(in_channels=in_channels, fn=self.norm_out, mask=False,
                                data_fmt='NLC' if self.force_nlc else self.data_fmt)
...
class Seq2Mat_Attn1DConv2DNet(nn.Layer):
    ...
    def __init__(self, args):
        ...
        self.Mat2DLinear = nn.Sequential(
            nn.Linear(in_channels, out_channels2d),
            nn.Swish(),
            nn.LayerNorm(out_channels2d),
        )
        in_channels = out_channels2d
...
class Seq2Mat_Attn1DConv2DF1Net(nn.Layer):
    """   """
    def __init__(self, args):
        super(Seq2Mat_Attn1DConv2DF1Net, self).__init__()
        ...
        if self.seq2mat_method in ['MULTIPLY']:
            self.Seq2MatPreLinear1 = nn.Linear(in_channels, in_channels)
            self.Seq2MatPreLinear2 = nn.Linear(in_channels, in_channels)
        ...
        self.LSTM1D = MyLSTM1DTower(args, in_channels=in_channels, data_fmt='NLC', return_states=True)
    ...
    def forward(self, x, seqs_len=None):
        ...
        x, (h, c) = self.LSTM1D(x, seqs_len=seqs_len)
...
class Seq2MatSeq_Attn1DConv2DConv1D2PNet(nn.Layer):
    """   """
    def __init__(self, args):
        super(Seq2MatSeq_Attn1DConv2DConv1D2PNet, self).__init__()
...
class Seq2Mat_LSTMConv2DF1Net(nn.Layer):
    """ This ignores all inter-residue information  """
    def __init__(self, args):
        super(Seq2Mat_LSTMConv2DF1Net, self).__init__()
        ...
        self.F1LSTM = MyLSTM1DTower(args, lstm_num=f1lstm_num, lstm_dim=f1lstm_dim,
            in_channels=in_channels, data_fmt='NLC', return_states=True)
    ...
    def forward(self, x, seqs_len=None):
        ...
        x, (h, c) = self.F1LSTM(x, seqs_len=seqs_len)
        # c: [num_layer*num_direction, batch_size, hidden_size]
        x = mi.reshape(mi.transpose(c, perm=[1, 0, 2]), [len(x), -1])
        f1 = F.sigmoid(self.F1Linear(x, seqs_len=seqs_len))
...
class SeqMat2Mat_Attn1DConv2DNet(nn.Layer):
    ...
    def __init__(self, args):
        ...
        if self.seq2mat_method in ['MULTIPLY', 'ADD']:
            self.Seq2MatPreLinear1 = nn.Linear(in_channels, in_channels)
            self.Seq2MatPreLinear2 = nn.Linear(in_channels, in_channels)
