...
def set_global_initializer(weight_fn, bias=0.0):
    """ Done in MyEmbeddingLayer, not sure whether the best/correct place for it """
    ...
    if weight_fn == 'kaimingnormal':
        ...
        logger.error(f'Unrecognized weight init fn: {weight_fn}!!!')

    logger.info('Set global initializer, weight: ' + \
                f'{misc.str_color(weight_fn, style="reverse")}, bias: {bias} ...')
...
def get_dropout_fn(dropout, data_fmt=None):
...
def stack_linear_block(channel_sizes, data_fmt=None, pre_act_norm=False, act_after_norm=False,
                act_fn=None, norm_fn=None, norm_axis=None, norm_mask=False,
    ...
    if act_fn is not None: act_fn = act_fn.lower()
    if norm_fn is not None: norm_fn = norm_fn.lower()
    ...
    for i in range(1, num_layers + 1):
        ...
        house_order = ['linear', 'act_norm_dropout']
        if pre_act_norm: house_order = house_order[::-1]
        for house_type in house_order:

            if house_type == 'linear':
                block_layers.append(nn.Linear(in_channels, channel_sizes[i])) 
                    # bias_attr=None, # False,
            ...
            if house_type == 'act_norm_dropout':

                room_order = ['act', 'norm']
                if act_after_norm: room_order = room_order[::-1]

                for room_type in room_order:
                    if room_type == 'act' and act_fn not in [None, 'none']:
                            block_layers.append(get_act_fn(act_fn))
                    if room_type == 'norm' and norm_fn not in [None, 'none']:
                        block_layers.append(MyNormLayer(in_channels=in_channels, method=norm_fn,
                                    axis=norm_axis, mask=norm_mask, data_fmt=data_fmt))

                if dropout > 0.0:
                    block_layers.append(get_dropout_fn(dropout, data_fmt=data_fmt))
...
def stack_lstm_block(channel_sizes, data_fmt=None, direction=None, 
            pre_act_norm=False, act_after_norm=False,
            act_fn=None, norm_fn=None, norm_axis=None, norm_mask=False,
            dropout=0.0, resnet=False, resnet_beta=None, is_return=False):
    ...
    for i in range(1, num_layers + 1):
        ...
        house_order = ['lstm', 'act_norm_dropout']
        if pre_act_norm: house_order = house_order[::-1]
        for house_type in house_order:
            ...
            if house_type == 'lstm':
                block_layers.append(nn.LSTM(
                            ...
                            num_layers = 1,
                            direction = direction,
                            dropout = 0.0, # this is inbetwen layers, not needed as num_layers = 0
                            ))
...
def stack_conv1d_block(channel_sizes, data_fmt=None, kernel_size=None, stride=None, dilation=None,
            padding=None, padding_mode=None, max_pool=None, pre_act_norm=False,
            act_after_norm=False, act_fn=None, norm_fn=None, norm_axis=None, norm_mask=False,
            dropout=0.0, resnet=False, resnet_beta=None, is_return=False):
    ...
    for i in range(1, num_layers + 1):
        ...
        house_order = ['conv1d_maxpool', 'act_norm_dropout']
        if pre_act_norm: house_order = house_order[::-1]
        for house_type in house_order:

            if house_type == 'conv1d_maxpool':
...
def stack_conv2d_block(channel_sizes, data_fmt=None, kernel_size=None, stride=None, dilation=None,
    ...
    in_channels = channel_sizes[0]
    for i in range(1, num_layers + 1):
        house_order = ['conv2d_maxpool', 'act_norm_dropout']
        if pre_act_norm: house_order = house_order[::-1]
        for house_type in house_order:

            if house_type == 'conv2d_maxpool':
...
class PosiEncoderxD(nn.Layer):
    """ """
    def __init__(self, in_channels, pe_dim=None, data_fmt=None, curve='trig', join_method='add',
        ...
        self.curve = curve.upper() if curve else None
        ...
        if self.curve == 'TRIG':
            ...
            logger.critical(f'Unknown posenc curve type: {self.curve}!!!')

        self.register_buffer('pos_mat', mi.empty([1], dtype='float32'), persistable=False)

        logger.info(f'======= {self.__class__.__name__} =======')
        logger.info(f'         data_fmt: {self.data_fmt}')
        logger.info(f'           PE dim: {self.pe_dim}')
        logger.info(f'            curve: {self.curve}')
    ...
    def forward(self, x, seqs_len=None, beta=1.0):
        ...
        if self.curve == 'TRIG':
            ...
            if self.pos_mat is None or x.ndim != self.pos_mat.ndim or \
                ...
                if x.ndim == 3:
                    self.pos_mat = mitas_utils.posencoder_trig_1d(x.shape[1:], curve=self.curve, mi=mi)
                elif x.ndim == 4:
                    self.pos_mat = mitas_utils.posencoder_trig_2d(x.shape[1:], curve=self.curve, mi=mi)
                else:
    ...
    def forward(self, data_len, seqs_len=None):
        ...
        with mi.no_grad():
            if self.attn_method in ['EFFICIENT']: 
                ...
                if len(data_len) == 1 or (len(data_len) > 1 and not self.force_nlc):
                ...
                elif len(data_len) == 2 and self.force_nlc:
            ...
            elif len(data_len) == 1 or (len(data_len) > 1 and not self.force_nlc):
                # 1D attention or axial attention over the last data_len
...
class MatDiagonalMask(nn.Layer):
    """  """
    def __init__(self, data_fmt='LL', dtype='float32', offset=0, reverse=False):
        """ the default is to mask out (0.) and diagnoal elements, unless reverse=True
        ...
        """
        super(MatDiagonalMask, self).__init__()
        self.dtype = dtype
        self.data_fmt = data_fmt.upper()
        self.register_buffer('mask_mat', mi.empty([0], dtype=self.dtype), persistable=False)
        self.mask_nil = np.empty(0, dtype=self.dtype)
...
class MyNormLayer(nn.Layer):
    def __init__(self, in_channels=None, data_fmt=None, mask=False, method=None, axis=None,
                epsilon=1e-6, affine=True, tracking_runing_stats=False, momentum=0.9):
        """ data_fmt: NLC/NCL """
        ...
        normfn_kwargs = dict(
            weight_attr=None if self.affine else False,
            bias_attr=None if self.affine else False,            
        )
        ...
        if self.mask:
            ...
            if self.method.startswith(('batch', 'inst')):
                need_mask_isa = True
                ...
                if len(self.axis) == 1: # can only be the last axis if one axis
                    ...
                    need_mask_isa = True
                    affine = False
            elif self.method.startswith('axis'):
                ...
                logger.critical(f'cannot recognize norm_fn: {self.method}')
        ...
        # ======= No masking: ndim <=3
            ...
            if self.method.startswith('batch'):
            ...
            elif self.method.startswith('inst'):
                ...
                logger.warning(f'cannot recognize norm_fn: {self.method}')
        # ======= No masking: ndim == 4
            ...
            logger.critical(f'Unable to initialize norm function with {self.method}')
    ...
    def forward(self, x, seqs_len):
        ...
        if len(self.data_fmt) <= 3: # NLC (and NC?)
            ...
            if self.method.startswith('batch') or \
                    (self.method.startswith('axis') and self.axis == [0, 1]):
                norm_axes = [0, 1]
                norm_consts = mi.sum(seqs_len)
            elif self.method.startswith('inst') or \
                    (self.method.startswith('axis') and self.axis == [1]):
                norm_axes = [1]
                norm_consts = mi.unsqueeze(seqs_len, axis=[1, 2])
            elif self.method.startswith('axis') and self.axis == [0]:
                norm_axes = [0]
                norm_consts = mi.sum(self.mask_mat[:batch_size, :data_len, :],
                    axis=0, keepdim=True) + self.epsilon
            elif self.method.startswith('layer') and len(self.axis) == 2:
                # only if last two dims are normalized
                norm_axes = [1, 2]
                norm_consts = mi.unsqueeze(in_channels * seqs_len, axis=[1, 2])
            else:
                logger.critical(f'Unknown norm: {self.method} axis: {self.axis} with masking!!!')
        # ======= Masking for ndim==4
            ...
            if self.method.startswith('batch') or \
                    (self.method.startswith('axis') and self.axis == [0, 1, 2]):
                norm_axes = [0, 1, 2]
                norm_consts = mi.sum(seqs_len ** 2)
            elif self.method.startswith('inst') or \
                    (self.method.startswith('axis') and self.axis == [1, 2]):
                norm_axes = [1, 2]
                ...
                norm_consts = mi.sum(self.mask_mat[:batch_size, :height, :width, :],
                                    axis=0, keepdim=True) + self.epsilon
            elif self.method.startswith('axis') and self.axis == [1]:
                norm_axes = [1]
                norm_consts = mi.unsqueeze(seqs_len, axis=[1, 2, 3])
            elif self.method.startswith('axis') and self.axis == [2]:
                norm_axes = [2]
                norm_consts = mi.unsqueeze(seqs_len, axis=[1, 2, 3])
            elif self.method.startswith('layer') and len(self.axis) == 2:
                norm_axes = [2, 3]
                norm_consts = mi.unsqueeze(in_channels * seqs_len, axis=[1, 2, 3])
            elif self.method.startswith('layer') and len(self.axis) == 3:
                norm_axes = [1, 2, 3]
                norm_consts = mi.unsqueeze(in_channels * (seqs_len ** 2), axis=[1, 2, 3])
            else:
                logger.critical(f'Unsupported method {self.method} with masking!!!')
        else:
...
class ScaledDotProductAttention(nn.Layer):
    ...
    def __init__(self, scale=None, act_fn='softmax', dropout=0.1, return_attn=True):
        """ """
        super().__init__()
...
class MyMultiHeadAttention(nn.Layer):
    ...
    def __init__(self, q_dim, k_dim, v_dim, nhead=2, temperature=None,
        ...
        if self.method in ['DOTPRODUCT', 'DOT_PRODUCT']:
            ...
            self.Attend = ScaledDotProductAttention(scale=self.scale,
                        act_fn=self.act_fn, dropout=self.dropout)
...
class MyAttnxDLayer(nn.Layer):
    def __init__(self, in_channels, nhead=2, axis=1, axis_share_attn=True,
                ...
                norm_fn='layer', norm_before=False,
        ...
        if self.join_method.startswith('ADD'):
            ...
            self.AttnNorm = MyNormLayer(in_channels, data_fmt=self.data_fmt,
                        method=self.norm_fn, axis=[-1], mask=False)
        ...
        if self.norm_before:
            self.FFNorm1 = MyNormLayer(in_channels, data_fmt=self.data_fmt, method=self.ff_norm_fn,
                        axis=[-1], mask=False)
        ...
        self.FFActivate = get_act_fn(self.ff_act_fn)
        # self.FFNorm2 = MyNormLayer(in_channels, data_fmt=self.data_fmt, method=self.ff_norm_fn,
        #                 axis=[-1], mask=False)        
    ...
    def attention_mono(self, q, mask=None):
    ...
    def attention_dual(self, q, k, mask=None):
    ...
    def attention_trio(self, q, k, v, mask=None):
        """ cross-attention between q and k=v """
    ...
    def forward(self, q, k=None, v=None, mask=None):
        ...
        if k is None and v is None:
            q_attn_out = self.attention_mono(q, mask=mask)
        elif v is None:
            q_attn_out = self.attention_dual(q, k, mask=mask)
        elif k is None:
            q_attn_out = 0.0
            logger.warning('not yet implemented')
        else:
            q_attn_out = self.attention_trio(q, k, v, mask=mask)
...
class MyEmbdxDLayer(nn.Layer):
    ...
    def __init__(self, args, in_channels=None, in_dtype=None, out_dtype=None, padding_idx=0,
        ...
        self.in_shapes.append(
            InputSpec(shape=(fmt2len_dict['N'],), dtype='int64', name='seqs_len')
            )

        self.embed_fn = None if args.embed_fn is None else args.embed_fn.lower()
        self.embed_dim = 8 if args.embed_dim is None else int(args.embed_dim)
        self.padding_idx = padding_idx if padding_idx is not None else args.embed_padding_idx
        # self.embed_num = int(args.embed_num)
        self.act_fn = None if args.embed_act_fn is None else args.embed_act_fn.lower()
        self.norm_fn = None if args.embed_norm_fn is None else args.embed_norm_fn.lower()
        self.norm_axis = args.embed_norm_axis
        self.norm_mask = args.embed_norm_mask
        ...
        logger.info(f'         norm_fn: {self.norm_fn}')
        logger.info(f'       norm_axis: {self.norm_axis}')
        logger.info(f'       norm_mask: {self.norm_mask}')
        logger.info(f'         out_fmt: {self.out_fmt}')
        ...
        if self.embed_fn in [None, 'none']:
            ...
            if self.embed_fn == 'embed':
                ...
                self.LayerList.append(nn.Sequential(*stack_linear_block(
                    ...
                    resnet = False,
                    resnet_beta = 1.0,
                    is_return = True, # turn off act/norm for the last layer (added below)
                    )))
                in_channels = self.embed_dim
            else:
        ...
        if self.act_fn not in [None, 'none']:
            self.LayerList.append(get_act_fn(self.act_fn))

        if self.norm_fn not in [None, 'none']:
            self.LayerList.append(MyNormLayer(in_channels=in_channels, method=self.norm_fn,
                axis=self.norm_axis, mask=self.norm_mask, data_fmt=in_fmt))
    ...
    def forward(self, x, seqs_len):
        ...
        #     y_truth = mi.(y_truth, dtype='float32')
        # if not isinstance(seqs_len, mi.Tensor) or seqs_len.dtype.name != 'INT32':
            # seqs_len = mi.(seqs_len, dtype='int32')
        # if not isinstance(seqs_len, mi.Tensor):
        #     seqs_len = mi.to_tensor(seqs_len, dtype='int64')

        if not isinstance(x, mi.Tensor) or not str(x.dtype).endswith(self.in_dtype):
            x = x.astype(self.in_dtype)
        # if not str(seqs_len.dtype).endswith('int64'):
        #     seqs_len = seqs_len.astype('int64')
        seqs_len = np.array(seqs_len, dtype='int64')
        if any(seqs_len == 0): 
...
class MyLinearTower(nn.Layer):
    def __init__(self, args, in_channels=None, data_fmt=None, is_return=False):
        ...
        self.norm_fn = args.norm_fn if args.linear_norm_fn is None else args.linear_norm_fn
        self.norm_axis = args.norm_axis if args.linear_norm_axis is None else args.linear_norm_axis
        self.norm_mask = args.norm_mask if args.linear_norm_mask is None else args.linear_norm_mask
        ...
        # add a buffer layer if the dimensions don't match for resnet
        # if self.linear_resnet and in_channels != self.linear_dim[0]:
        #     self.linear_layers.extend(stack_linear_block(
        #             [in_channels, self.linear_dim[0]],
...
class MyLSTM1DTower(nn.Layer):
    def __init__(self, args, in_channels=None, data_fmt='NLC', is_return=False,
        ...
        self.num_blocks = int(args.lstm_num)
        self.channel_sizes = [int(_i) for _i in args.lstm_dim] \
        ...
        self.norm_fn = args.norm_fn if args.lstm_norm_fn is None else args.lstm_norm_fn
        self.norm_axis = args.norm_axis if args.lstm_norm_axis is None else args.lstm_norm_axis
        self.norm_mask = args.norm_mask if args.lstm_norm_mask is None else args.lstm_norm_mask
        ...
        self.train_initial = args.lstm_train_initial
    ...
    def forward(self, x, seqs_len):
        ...
        # seqs_len = kwargs.get('seqs_len', None)
        # seqs_len = mi.full((batch_size,1), input_len, dtype='int64')
        ...
        for OneLayer in self.LayerList:
            if isinstance(OneLayer, nn.LSTM):
                ...
                x = OneLayer(x)
                # logger.error('Unknown Layer type in MyLSTMTower!!!')
...
class MyConv1DTower(nn.Layer):
    def __init__(self, args, in_channels=None, data_fmt=None, is_return=False):
        ...
        self.norm_fn = args.norm_fn if args.conv1d_norm_fn is None else args.conv1d_norm_fn
        self.norm_axis = args.norm_axis if args.conv1d_norm_axis is None else args.conv1d_norm_axis
        self.norm_mask = args.norm_mask if args.conv1d_norm_mask is None else args.conv1d_norm_mask
...
class MyConv2DTower(nn.Layer):
    def __init__(self, args, in_channels=None, data_fmt=None, is_return=False):
        ...
        self.norm_fn = args.norm_fn if args.conv2d_norm_fn is None else args.conv2d_norm_fn
        self.norm_axis = args.norm_axis if args.conv2d_norm_axis is None else args.conv2d_norm_axis
        self.norm_mask = args.norm_mask if args.conv2d_norm_mask is None else args.conv2d_norm_mask
...
class MyAttnxDTower(nn.Layer):
    def __init__(self, args, in_channels=None, data_fmt=None, attn_dual=False,  attn_trio=False,
                 attn_axis=None, attn_method=None,
                 mask='auto', norm_out=False):
        ...
        if self.attn_method == 'PADDLE':
            ...
            self.AttnLayers.append(nn.TransformerEncoder(
                attn_layer,
            ...
            for i in range(self.num_blocks):
                ...
                self.AttnLayers.append(MyAttnxDLayer(
                    in_channels,
                    nhead=self.attn_nhead,
                    axis=self.attn_axis,
                    data_fmt=data_fmt,
                ...
                if self.attn_dual: # more than one inputs

                    pass


        # close out
        for i, AttnLayer in enumerate(self.AttnLayers):
            self.add_sublayer(f'Layer{i}', AttnLayer)

        if self.norm_out:
            self.NormOut = MyNormLayer(in_channels,  method=self.norm_out, mask=False,
                                        data_fmt='NLC' if self.force_nlc else self.data_fmt)
    ...
    def forward(self, q, k=None, v=None, seqs_len=None):
        ...
        for i, AttnLayer in enumerate(self.AttnLayers):
            if isinstance(AttnLayer, nn.TransformerEncoder):
                # paddle doesn't accept empty mask
                q = AttnLayer(q, mask if mask is not None and len(mask) else None)
            elif isinstance(AttnLayer, MyAttnxDLayer):
                q = AttnLayer(q, k, v, mask=mask)
            else: # AxialAttention doesnot support mask or batch_size > 1
                q = AttnLayer(q, seqs_len)
        ...
        if self.norm_out:
            q = self.NormOut(q, seqs_len=seqs_len)
...
def MyInitTower(args, in_channels=None, data_fmt=None):
    return MyLinearTower(misc.Struct(vars(args)).update(
...
def MyReturnTower(args, in_channels=None, data_fmt=None):
    return MyLinearTower(misc.Struct(vars(args)).update(
...
class LinearNet(nn.Layer):
    ...
    def __init__(self, args):
        ...
        in_channels = self.in_channels # keep record of current channel dim
...
class Seq2Seq_AttnConvNet(nn.Layer):
    def __init__(self, args):
        super(Seq2Seq_AttnConvNet, self).__init__()
...
class SeqMat2Mat_Attn1DConv2DNet(nn.Layer):
    ...
    def __init__(self, args, seq2mat_method='multiply'):
        ...
        in_channels += 4 # add the bpmat [LL4] here!!!

        out_channels = (in_channels - 4) // self.Seq2Mat.mul_channels
...
class Seq2Mat_XCodeConv2DNet(nn.Layer):
    ...
    def __init__(self, args, seq2mat_method='multiply'):
        ...
        self.Attn = MyAttnxDTower(args, in_channels=in_channels, data_fmt='NLC',
                        attn_dual=True, attn_method='dotproduct', attn_axis=2)
        in_channels = self.Attn.out_channels
...
class SeqMat2Mat_AttnLSTMConv2DNet(nn.Layer):
    ...
    def __init__(self, args):
        ...
        # 1) Sequence pre-layers, starts with [N, L, C]
        seq_in_channels = self.in_channels # keep record of current channel dim
